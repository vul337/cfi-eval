...
impl TryFrom<&OsStr> for Command {
    /// Program name is missing. (`args` doesn't has any item.)
    type Error = ();

    /// Constructs a new `Command`, by splitting the arguments by space.
    fn try_from(args: &OsStr) -> Result<Command, Self::Error> {
        let bytes = args.as_bytes();

        fn skip_blank(buf: &[u8], pos: usize) -> usize {
            if pos > buf.len() {
                return buf.len();
            }
            buf[pos..]
        ...
        fn find_blank(buf: &[u8], pos: usize) -> usize {
            if pos > buf.len() {
                return buf.len();
            }
            buf[pos..]
        ...
        loop {
            ...
            let begin = skip_blank(bytes, i);
            let end = find_blank(bytes, begin + 1);
            if end == begin {
                break;
            }

            // If the program is not known yet, construct the command; add an
            // argument otherwise.
            let arg = OsStr::from_bytes(&bytes[begin..end]);
        ...
        cmd.map(|cmd| Ok(cmd)).unwrap_or(Err(()))
    }
}
...
impl Command {
    ...
    /// Constructs a new `Command`, where the program and args are `args[0],
    /// args[1], ...`.
    ///
    /// Returns `Err(())` if program name is missing. (`args` doesn't has any
    /// item.)
    pub fn try_from_args<U: AsRef<OsStr>, I: IntoIterator<Item = U>>(
        args: I,
    ) -> Result<Command, ()> {
...
mod test {
    ...
    fn check_abc(s: &'static str) {
        let cmd = Command::try_from(OsString::from(s).as_os_str()).unwrap();
        assert_eq!(cmd.program.as_bytes(), "a".as_bytes());
        assert_eq!(cmd.args.len(), 2);
        assert_eq!(cmd.args[0].as_bytes(), "b".as_bytes());
        assert_eq!(cmd.args[1].as_bytes(), "c".as_bytes());
    }
    ...
    fn process_from_str() {
        ...
        check_abc("a   b   c");
        check_abc("  a b   c  ");
        Command::try_from(OsString::from("    ").as_os_str()).unwrap_err();
    }
}
