...
impl RegAccessType {
    ...
    pub fn is_readable(&self) -> bool {
        if let RegAccessType::Read | RegAccessType::ReadWrite = self {
            true
        } else {
            false
        }
    ...
    pub fn is_writable(&self) -> bool {
        if let RegAccessType::Write | RegAccessType::ReadWrite = self {
            true
        } else {
            false
        }
...
impl TryFrom<cs_ac_type> for RegAccessType {
    ...
    fn try_from(access: cs_ac_type) -> Result<Self, Self::Error> {
        ...
        Ok(match (is_readable, is_writable) {
            (true, false) => RegAccessType::Read,
            (false, true) => RegAccessType::Write,
            (true, true) => RegAccessType::ReadWrite,
            _ => unreachable!(),
        })
    }
}
